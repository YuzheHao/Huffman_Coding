...
struct data
{
	...
	struct data *next = (struct data *)malloc(sizeof(struct data));; //指向其运算父节点
	struct data *bubnext = (struct data *)malloc(sizeof(struct data));; //指向冒泡排序的下一个人
};
...
int main(void)
{
	...
	//循环部分开始///////////////////////////////////////////////////////
	struct data *seafloor;
	seafloor = &node[num - 1];
	seafloor->bubnext = node[num - 1].bubnext;
	struct data sum[M]; //等一下用的合成节点
	...
	while (psum != 1)
	{
		...
		//新节点作单次冒泡处理///////////////////////////////////////////////////
		struct data *newnode; //始终指向新节点
		newnode = seafloor;
		struct data *comp; //记录当前正在比较的节点
		comp = seafloor->bubnext; //初始化
		struct data *dummy1; //用于临时储存在海底上一层的节点位置
		dummy1 = (struct data *)malloc(sizeof(struct data));
